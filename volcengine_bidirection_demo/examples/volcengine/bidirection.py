...
def play_audio_file(filename):
    ...
    try:
        ...
        if system == "Windows":
            ...
            try:
                subprocess.run(["wmplayer", filename], shell=True, capture_output=True)
                logger.info(f"使用wmplayer播放: {filename}")
            except:
                # 备用方案：直接用系统默认播放器
                os.startfile(filename)
                logger.info(f"使用系统默认播放器播放: {filename}")
        elif system == "Darwin":  # macOS
            # 使用afplay后台播放
            subprocess.run(["afplay", filename], capture_output=True)
        else:  # Linux
            # 使用aplay或paplay后台播放
            try:
                subprocess.run(["aplay", filename], capture_output=True)
            except:
                subprocess.run(["paplay", filename], capture_output=True)
        logger.info(f"音频播放完成: {filename}")
